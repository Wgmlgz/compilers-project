...
void immutableError(const std::string& message, const SourceLocation& location) {
    auto error = createError(ErrorType::IMMUTABLE_ERROR, message, location);
    
    // Example of how to add a custom fix
    error.suggest_fix("use 'let mut' for variables that need to be reassigned", 
...
void formatError(const std::string& message, const SourceLocation& location) {
    reportError(ErrorType::FORMAT_ERROR, message, location);
}

void functionError(const std::string& message, const SourceLocation& location) {
    reportError(ErrorType::FUNCTION_ERROR, message, location);
}
